...
trait WeddingPlannerService extends LService {
  ...
  val sample = Task {
    graph.edges().toList.foreach(_.remove())
    graph.values().toList.foreach(_.remove())
    graph.nodes().toList.foreach(_.remove())
  ...
  object LabelApi extends Endpoint.Module[IO] {
    ...
    val resetGraphs: Endpoint[IO, String] = get(path("reset")) {
      sample.forkAndForget.runToFuture(monix.execution.Scheduler.global)
  ...
  val api = labels :+: agendaService.api :+: appointmentService.api :+: personService.api :+: placeService.api :+: reportToMarriageService.api :+: weddingReservationService.api :+: utils.persist :+: LabelApi.clearGraphs :+: LabelApi.resetGraphs

  sample.runSyncUnsafe()(monix.execution.Scheduler.global, CanBlock.permit)
